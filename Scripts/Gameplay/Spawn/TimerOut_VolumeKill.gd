...
func _ready() -> void:
	...
	targetPosition = iniPosition + translateOffset
	
	Manager.gameManager.game_timer.timeout.connect(StartTranslation)
	Manager.gameManager.FightEnd.connect(StopTranslation)
...
func Translate(targetTranslation:Vector3):
	...
	while timer.time_left > 0.0 and inTranslation:
